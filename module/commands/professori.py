...
def prof(update: Update, context: CallbackContext) -> None:
    ...
    message_text: str = generate_prof_text(
        update.message.from_user.language_code, context.args
    )
    ...
    if len(professors) == 1:
        ...
        if id_photo != "Non presente":
            ...
            context.bot.sendPhoto(chat_id=update.message.chat_id, photo=photo_url)
