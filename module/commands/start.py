...
def start(update: Update, context: CallbackContext) -> None:
    ...
    """
    reply_keyboard: ReplyKeyboardMarkup = get_help_keyboard(
        update.message.from_user.language_code
    )
    message_text: str = get_locale(
        update.message.from_user.language_code, TEXT_IDS.START_TEXT_ID
    )
    context.bot.sendMessage(
        chat_id=update.message.chat_id, text=message_text, reply_markup=reply_keyboard
    )
