...
class TestDb:
    ...
    def test_query_from_string(self) -> None:
        ...
        DbManager.query_from_string(
            "CREATE TABLE IF NOT EXISTS temp(id INT PRIMARY KEY);"
        )
    ...
    def test_select_from(self, test_results: dict) -> None:
        ...
        curr_results = DbManager.select_from(
            table_name=TABLE_NAME, where="id = ? OR id = ?", where_args=(2, 4)
        )
        assert curr_results == test_results["select_from_1"]

        curr_results = DbManager.select_from(
            table_name=TABLE_NAME, select="id, string2", where="id = 3"
        )
    ...
    def test_count_from(self, test_results: dict) -> None:
        ...
        num_rows = DbManager.count_from(
            table_name=TABLE_NAME, where="id > ?", where_args=(2,)
        )
        assert num_rows == test_results["count_from_1"]

        num_rows = DbManager.count_from(
            table_name=TABLE_NAME, select="string2", where="id = ?", where_args=(3,)
        )
    ...
    def test_insert_into(self, test_results: dict) -> None:
        ...
        DbManager.insert_into(
            ...
            values=(7, "test_insert_into1", "TEST_INSERT_INTO1"),
            columns=("id", "string1", "string2"),
        )
        DbManager.insert_into(
            table_name=TABLE_NAME,
            values=(8, "test_insert_into2", "TEST_INSERT_INTO2"),
            columns=("id", "string1", "string2"),
        )
        DbManager.insert_into(
            table_name=TABLE_NAME,
            values=(
                (9, "test_insert_into3", "TEST_INSERT_INTO3"),
                (10, "test_insert_into4", "TEST_INSERT_INTO4"),
            ),
            columns=("id", "string1", "string2"),
            multiple_rows=True,
        )
        ...
        curr_results = DbManager.select_from(
            table_name=TABLE_NAME, where="id >= ?", where_args=(7,)
        )
